...
int main() {
  ...
  while (1) {
    ...
    if (is_declaration(input)) {
      ...
      printf("Add to global code: %s\n", input);
      continue;
    }

    // 构建完整的代码
    snprintf(current_code, sizeof(current_code),
             "%s%s\nint main() {\n%s\nreturn 0;\n}\n", includes, global_code,
             input);
...
int is_declaration(const char *code) {
  ...
  // 检查是否是类型声明
  if (strncmp(trimmed, "int ", 4) == 0 || strncmp(trimmed, "float ", 6) == 0 ||
      strncmp(trimmed, "double ", 7) == 0 ||
      strncmp(trimmed, "char ", 5) == 0 || strncmp(trimmed, "void ", 5) == 0) {
...
int compile_and_run(const char *code) {
  ...
  snprintf(compile_cmd, sizeof(compile_cmd), "gcc -o %s %s 2>&1",
           TEMP_EXEC_NAME, TEMP_FILE_NAME);

  compile_result = system(compile_cmd);
  char *compile_error = strerror(errno);

  if (compile_result != 0) {
    printf("Compilation false, error: %s\n", compile_error);
    return -1;
  }
